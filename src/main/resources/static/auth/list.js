$(document).ready(function () {
    selectMenu("fourMenu-id|authMenu-id");
        ...
        columns: [
            ...
            },
            {
                "data": null, "render": function (data, type, row) {
                    return '<a href="javascript:void(0);"  onclick="edit(\'' + row.userCode + '\')">编辑</a>'
                        + '&nbsp;&nbsp;&nbsp;'
                        + '<a href="javascript:void(0);"  onclick="del(\'' + row.userCode + '\');">删除</a>';
                }
            }
        ],
...
function view(userCode) {
    $('#modal-auth').load("/user/detail", {userCode: userCode}).modal();
}
...
function del(userCode) {
    bootbox.confirm("您确定要删除吗", function (result) {
        if (result) {
            $.post("/auth/save", {userCode: userCode, role: 0}, function (result) {
                debugger;
                if (result.code === 1) {
                    toastr.success("操作成功");
